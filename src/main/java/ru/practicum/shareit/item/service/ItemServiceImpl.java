...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDetailsDto getById(Long itemId) {
        Item item = getItem(itemId);
        return ItemMapper.toItemDetailsDto(item, getComments(itemId));
    }
    ...
    public List<ItemDto> getAll(Long userId) {
        ...
        Map<Long, List<Booking>> bookingsByItemId = getBookingsByItemIds(itemIds);
        Map<Long, List<CommentDto>> commentsByItemId = getCommentsByItemIds(itemIds);
        return items.stream()
                .map(item -> ItemMapper.toItemDto(
                        item,
                        getLastBooking(bookingsByItemId.get(item.getId()), now),
                        getNextBooking(bookingsByItemId.get(item.getId()), now),
    ...
    private Map<Long, List<Booking>> getBookingsByItemIds(List<Long> itemIds) {
        return bookingRepository.findAllByItem_IdInAndStatusOrderByStartDesc(itemIds, BookingStatus.APPROVED).stream()
                .collect(Collectors.groupingBy(booking -> booking.getItem().getId()));
    }

    private Map<Long, List<CommentDto>> getCommentsByItemIds(List<Long> itemIds) {
        return commentRepository.findAllByItem_IdInOrderByCreatedDesc(itemIds).stream()
                .collect(Collectors.groupingBy(
                        comment -> comment.getItem().getId(),
                        Collectors.mapping(ItemMapper::toCommentDto, Collectors.toList())));
    }

    private LocalDateTime getLastBooking(List<Booking> bookings, LocalDateTime now) {
        if (bookings == null) {
            return null;
        }
    ...
    private LocalDateTime getNextBooking(List<Booking> bookings, LocalDateTime now) {
        if (bookings == null) {
            return null;
        }
        return bookings.stream()
